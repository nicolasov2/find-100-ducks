...
const POSITIONS: readonly Vector3Tuple[] = [
  ...
  [CX - 2, 0.02, -5.5], [CX + 0.5, 0.02, -5.5],
  // ── Beside fridge — visible from side (3) ──
  [CX + 7.2, 0.02, -5.5], [CX + 6.5, 0.02, -5.5], [CX + 6.0, 0.02, -6.5],
  // ── On top of fridge (1) ──
  [CX + 7.2, 2.12, -6.2],
  // ── Oven stovetop + floor beside (3) ──
  [CX + 2.5, 1.00, -6.2], [CX + 2.8, 0.02, -5.5], [CX + 3.3, 0.02, -6.0],
  // ── Under/on round table (4) ──
  [CX, 0.87, 2.0], [CX + 0.5, 0.02, 2.5], [CX - 0.5, 0.02, 1.5], [CX, 0.02, 3.0],
  // ── On dining chairs (4) ──
  ...
  [CX + 0.5, 1.12, -6.2],
  // ── Beside trash cans (2) ──
  [CX + 7.3, 0.02, 5.8], [CX - 7.3, 0.02, -1.8],
  // ── Beside boxes — floor (3) ──
  [CX - 6.5, 0.02, -1.8], [CX - 6.8, 0.02, 3.5], [CX + 5.3, 0.02, 5.2],
  // ── On boxes (2) ──
  [CX - 7.0, 0.45, 4.0], [CX + 5.0, 0.45, 5.5],
  // ── Plants — beside (3) ──
  [CX + 7.0, 0.02, 6.0], [CX - 7.0, 0.02, 6.0], [CX - 7.0, 0.02, -5.8],
  // ── Floor cushion (1) ──
  [CX - 5, 0.24, 4],
  // ── Wall corners — accessible floor (6) ──
  [CX - 7.5, 0.02, -6.5], [CX + 7.5, 0.02, 6.5], [CX - 7.5, 0.02, 6.5],
  [CX + 7.5, 0.02, -6.5], [CX - 7.5, 0.02, 0], [CX + 7.5, 0.02, 0],
  // ── Open floor (6) ──
  [CX - 3, 0.02, 0], [CX + 3, 0.02, 0], [CX, 0.02, -3],
  [CX - 5, 0.02, 5], [CX + 5, 0.02, 4], [CX, 0.02, 5],
  // ── Pot rack — hanging in pots (4) ──
  [CX, 2.50, 1.0], [CX - 0.5, 2.50, 1.0], [CX - 4, 2.50, -2], [CX - 4.5, 2.50, -2.0],
  // ── Beside brooms — corner (2) ──
  [CX + 7.0, 0.02, 6.0], [CX - 7.0, 0.02, -6.0],
  // ── On herb planter (3) ──
